...
movieRouter.get('/', (req,res) => {
    ...
    if(query) {
        ...
        if(listMovieQuery.length > 0) {
            ...
            return res.send({success: false, message: "Not Found Movie"});
        }
    }
    else {
...
movieRouter.get('/:title', (req,res) => {
    ...
    if(movie === null || movie === undefined) { 
        return res.send({success: false, message: "Not Found Club"});
    }
    else { 
        return res.send({success: true, message: "Found Club", data: movie});
    }
})
...
movieRouter.post('/', (req,res) => {
    ...
    if(!title || title.trim() === "" || title.length === 0) {
        return res.send({success: false, message: "Invalid or missing title"});
    }
    // Check Exists
    else if(movie !== null && movie !== undefined) {
        return res.send({success: false, message: "Title is exists"});
    }
    // Add Movie
    else{
        const newMovie = req.body;
        if(newMovie === null || newMovie === undefined) { 
            return res.send({success: false, message: "Add Movie Failed"});
        }
        else { 
            movies.add(newMovie);
            return res.send({success: true, message: "Add Movie Success", data: newMovie}); 
        }
    }
})
...
movieRouter.put('/:title', (req,res) => {
    ...
    if(movie === null || movie === undefined) {
        ...
        return res.send({success: false, message: "Update Movie Failed"});
    }
    else { 
        movies.update(title,newMovie);
        return res.send({success: true, message: "Update Movie Success", data: newMovie}); 
    }
})
...
movieRouter.delete('/:title', (req,res) => {
    ...
    if(movie === null || movie === undefined) {
        ...
        movies.remove(title);
        return res.send({success: true, message: "Delete Movie Success"}); 
    }
})
